...
# Main driver function. Runs PCA, initializes reference vectors, and runs the clustering until convergence. Includes the plotting
def myKMeans(data_clusterX, target_clusterY, k):
    
    if np.size(data_clusterX,1) > 3:
        pca = PCA(n_components=3)
        data_clusterX = pca.fit_transform(data_clusterX)

    ##dictionary for the reference vectors.  The value comes from a random observation in the data.
    centroids = {
        i+1: data_clusterX[random.randrange(0,np.size(data_clusterX,axis=0))]
        for i in range(k)
    }
    ...
    #Loop until convergence (currently set to a lower number so it wouldnt take too long)
    while sums > e:
        if iters < frames:
            clusters_data = clustering(data_clusterX, centroids )
